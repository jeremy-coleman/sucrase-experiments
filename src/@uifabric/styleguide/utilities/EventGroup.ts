...
export class EventGroup {
  ...
  public static raise(
    ...
    if (EventGroup._isElement(target)) {
      ...
      while (target && retVal !== false) {
        let events = target.__events__ as IEventRecordsByName;
        let eventRecords = events ? events[eventName] : null;

        if (eventRecords) {
          for (let id in eventRecords) {
            if (eventRecords.hasOwnProperty(id)) {
              let eventRecordList = eventRecords[id] as IEventRecord[];
  ...
  public static isObserved(target: any, eventName: string): boolean {
    let events = target && target.__events__ as IEventRecordsByName;
  ...
  public static isDeclared(target: any, eventName: string): boolean {
    let declaredEvents = target && target.__declaredEvents as IDeclaredEventsByName;
  ...
  public on(target: any, eventName: string, callback: (args?: any) => void, options?: boolean | AddEventListenerOptions): void {
    if (eventName.indexOf(',') > -1) {
      ...
      let events = (target.__events__ = target.__events__ || {}) as IEventRecordsByName;
      events[eventName] =
        events[eventName] || {count: 0} as IEventRecordList;
      events[eventName][this._id] = events[eventName][this._id] || [];
      (events[eventName][this._id] as IEventRecord[]).push(eventRecord);
      ...
      if (EventGroup._isElement(target)) {
        ...
        if (target.addEventListener) {
          ...
          (target as EventTarget).addEventListener(eventName, processElementEvent, options);
  ...
  public off(target?: any, eventName?: string, callback?: (args?: any) => void, options?: boolean | AddEventListenerOptions): void {
    for (let i = 0; i < this._eventRecords.length; i++) {
      ...
      if (
        ...
        let events = eventRecord.target.__events__ as IEventRecordsByName;
        let targetArrayLookup = events[eventRecord.eventName];
        let targetArray = targetArrayLookup ? (targetArrayLookup[this._id] as IEventRecord[]): null;
