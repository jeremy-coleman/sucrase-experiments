...
const processor = postcss.plugin('postcss-modules-scope', function(options) {
  return css => {
    ...
    function exportScopedName(name, rawName) {
      ...
      _exports[key] = _exports[key] || [];

      if (_exports[key].indexOf(value) < 0) {
        _exports[key].push(value);
      }
        ...
        classes.forEach(className => {
          ...
          if (global) {
            localNames.forEach(exportedName => {
              _exports[exportedName].push(global[1]);
            });
          } else if (hasOwnProperty.call(importedNames, className)) {
            localNames.forEach(exportedName => {
              _exports[exportedName].push(className);
            });
          } else if (hasOwnProperty.call(_exports, className)) {
            localNames.forEach(exportedName => {
              _exports[className].forEach(item => {
                _exports[exportedName].push(item);
              });
            });
          } else {
            throw decl.error(
    ...
    const exportedNames = Object.keys(_exports);
    ...
    if (exportedNames.length > 0) {
      ...
      exportedNames.forEach(exportedName =>
        exportRule.append({
          prop: exportedName,
          value: _exports[exportedName].join(' '),
          raws: { before: '\n  ' },
        })
      );
