...
//replaced browser-pack , makes no sense to have it as default when tinyify uses pack flat
import bpack from './packflat'
import bresolve from './resolvers/browser-resolve'
import cachedPathRelative from './resolvers/cached-path-relative'
import depsSort from './resolvers/deps-sort'
import insertGlobals from './resolvers/insert-module-globals'
import mdeps from './resolvers/module-deps'
import resolve from './resolvers/resolve'
import { concat, readonly, splicer, through } from './streamz'
...
function createCompileError() {
	function CompileError(err, src, file) {
    ...
    CompileError.prototype.toString = () => this.annotated;
    CompileError.prototype.inspect = () => this.annotated;

	return CompileError;
};

const ParseError = createCompileError()

class ParseError1 extends SyntaxError {
    line: any
    column: any
    annotated: string
...
Browserify.prototype.require = function (file, opts) {
    ...
    if (isStream(file)) {
        ...
        file.pipe(concat(function (buf) {
            ...
            if (expose || opts.entry === false) {self._expose[id] = filename;}
