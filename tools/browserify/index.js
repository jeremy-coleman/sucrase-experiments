...
const {splicer, readonly, through, concat} = require('./streams')
// var splicer = require('./main/labeled-stream-splicer');
// var readonly = require('./main/read-only-stream');
// var through = require('./main/through2');
// var concat = require('./main/concat-stream');
...
//var xtend = require('./main/xtend');
//var defined = require('./main/defined');
//var has = require('has');
//var sanitize = require('./main/htmlescape').sanitize;
...
//var cachedPathRelative = require('./main/module-deps').cachedPathRelative
...
class Browserify extends EventEmitter {
    ...
    if (typeof files === 'string' || isArray(files) || isStream(files)) {
        opts = Object.assign({}, opts, { entries: [].concat(opts.entries || [], files) });
    }
    else opts = Object.assign({}, files, opts);
...
require = function (file, opts) {
    ...
    if (isArray(file)) {
        file.forEach(function (x) {
            if (typeof x === 'object') {
                self.require(x.file, Object.assign({}, opts, x));
            }
    ...
    if (typeof file === 'object') {
        row = Object.assign({}, file, opts);
    }
    else if (!opts.entry && isExternalModule(file)) {
        // external module or builtin
        row = Object.assign({}, opts, { id: expose || file, file: file });
    }
    else {
        row = Object.assign({}, opts, { file: path.resolve(basedir, file) });
    }
...
external = function (file, opts) {
    var self = this;
    if (isArray(file)) {
        file.forEach(function (f) {
            if (typeof f === 'object') {
                self.external(f, Object.assign({}, opts, f));
            }
    ...
    if (file && typeof file === 'object' && typeof file.bundle === 'function') {
        ...
        b.pipeline.get('deps').push(through.obj(function (row, enc, next) {
            bdeps = Object.assign({}, bdeps, row.deps);
            this.push(row);
            next();
        }));
...
_createPipeline = function (opts) {
    ...
    this._bpack = bpack(Object.assign({}, opts, { raw: true }));
...
_createDeps = function (opts) {
    var self = this;
    var mopts = Object.assign({}, opts);
    var basedir = defined(opts.basedir, process.cwd());
    ...
    else mopts.modules = Object.assign({}, builtins);
    ...
    function globalTr (file) {
        ...
        if (opts.commondir === false && opts.builtins === false) {
          opts.insertGlobalVars = Object.assign({},{
        ...
        return insertGlobals(file, Object.assign({}, opts, {
            debug: opts.debug,
...
reset = function (opts) {
    ...
    this.pipeline = this._createPipeline(Object.assign({}, opts, this._options));
...
var browserifyFactory = (file, opts = undefined) => new Browserify(file, opts)

module.exports = browserifyFactory