...
const paths = {
	...
	icon: path.join(app.getAppPath(), 'src/app-web/icon/icon.png')
	//icon: path.join(__dirname, '..', 'app-web', 'icons', 'icon.png')
};
...
const production = 
__dirname.includes('Contents/Resources/app') || __dirname.includes('resources/app') || __dirname.includes('resources\\app');
...
app.on('ready', () => {
	...
	let appPath = path.join(app.getAppPath(), 'src/app-web')
	...
	if (!production) {
		...
		fs.watch(appPath, {persistent: true, recursive: true}, (event, filename) => {
