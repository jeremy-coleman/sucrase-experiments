...
// function example(moduleMeta) {
//   var moduleIndexesToNames = {};
//   Object.entries(moduleMeta).forEach(([name, value]) => {
//   //forOwn(moduleMeta, function(value, name) {
//     return moduleIndexesToNames[value.index] = name
//   })
//   //});
    ...
    var moduleHotApply = function(options, cb) {
      ...
      for (i=0, len=acceptedUpdates.length; i<len; i++) {
        ...
        if (!has(runtimeModuleInfo, an)) {
          ...
          runtimeModuleInfo[an].selfAcceptCbs.forEach(function(cb) {
            selfAccepters.push({name: an, cb: cb});
          })
        ...
        //     //console.log(localHmr.newLoad.moduleDefs[localHmr.newLoad.moduleMeta[an].index][1])

        //   Object.assign({}, Object.entries(localHmr.newLoad.moduleDefs[localHmr.newLoad.moduleMeta[an].index][1]).map(([depRef, depIndex]) => {
        ...
        //       console.log('return value',localHmr.runtimeModuleInfo[depName].index)
        //       return localHmr.runtimeModuleInfo[depName].index;
        //     } 
        //     else {
        //       console.log('return value simple',depName)
        //       return depName;
        //     }
        //   }))
        // ];
        ...
        moduleDefs[runtimeModuleInfo[an].index] = [
          ...
          mapValues(localHmr.newLoad.moduleDefs[localHmr.newLoad.moduleMeta[an].index][1], function(depIndex, depRef) {
            //console.log(localHmr.newLoad.moduleDefs[localHmr.newLoad.moduleMeta[an].index][1])
            //Object.entries(localHmr.newLoad.moduleDefs[localHmr.newLoad.moduleMeta[an].index][1]).map(([k,v]) => console.log(v))
      ...
      // Call the self-accepting modules
    selfAccepters.forEach(function(obj) {
      try {
          require(runtimeModuleInfo[obj.name].index);
          } 
      catch(e) {
          if (obj.cb) {obj.cb.call(null, e)}
          else {
                  if (errCanWait) emitError(errCanWait);
                  errCanWait = e;
                }
    ...
    var setupSocket = function() {
      ...
      var acceptNewModules = function(msg) {
        ...
        var newModuleMeta = localHmr.newLoad ?
          ...
          //new
        Object.entries(msg.newModuleData).forEach(([key, value]) => {
            //forOwn(moduleMeta, function(value, name) {
                newModuleMeta[key] = {
                  index: value.index,
                  hash: value.hash,
        ...
        msg.removedModules.forEach(function(removedName) {
          delete newModuleDefs[runtimeModuleInfo[removedName].index];
          delete newModuleMeta[removedName];
        });
